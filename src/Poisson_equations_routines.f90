              ...
              IF(EQUATIONS_MATERIALS%MATERIALS_FIELD_AUTO_CREATED) THEN
                ...
                IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_CONSTANT_SOURCE_POISSON_SUBTYPE) THEN
                  ...
                  ELSEIF (NUMBER_OF_DIMENSIONS==2) THEN
                    NUMBER_OF_MATERIALS_COMPONENTS=4;
                ...
                ELSEIF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_LINEAR_SOURCE_POISSON_SUBTYPE) THEN
                  ...
                  ELSEIF (NUMBER_OF_DIMENSIONS==2) THEN
                    NUMBER_OF_MATERIALS_COMPONENTS=5;
                  ELSEIF (NUMBER_OF_DIMENSIONS==3) THEN
                    NUMBER_OF_MATERIALS_COMPONENTS=8;
                  ENDIF 
                ENDIF
                !Set the number of materials components
                CALL FIELD_NUMBER_OF_COMPONENTS_SET_AND_LOCK(EQUATIONS_MATERIALS%MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE, &
                ...
                IF(EQUATIONS_SET%SUBTYPE==EQUATIONS_SET_CONSTANT_SOURCE_POISSON_SUBTYPE) THEN
                  IF(NUMBER_OF_DIMENSIONS==1) THEN
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,2,ERR, &
                      & ERROR,*999)
                  ELSEIF(NUMBER_OF_DIMENSIONS==2) THEN
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,4,ERR, &
                      & ERROR,*999)
                  ELSEIF(NUMBER_OF_DIMENSIONS==3) THEN
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,7,ERR, &
                      & ERROR,*999)
                  ENDIF              
                ELSE
                  IF(NUMBER_OF_DIMENSIONS==1) THEN
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,3,ERR, &
                      & ERROR,*999)
                  ELSEIF(NUMBER_OF_DIMENSIONS==2) THEN
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,5,ERR, &
                      & ERROR,*999)
                  ELSEIF(NUMBER_OF_DIMENSIONS==3) THEN
                    CALL FIELD_NUMBER_OF_COMPONENTS_CHECK(EQUATIONS_SET_SETUP%FIELD,FIELD_U_VARIABLE_TYPE,8,ERR, &
                      & ERROR,*999)
                  ENDIF 
                ENDIF
              ENDIF
...
999 CALL ERRORS("POISSON_EQUATION_EQUATIONS_SET_NONLINEAR_SOURCE_SETUP",ERR,ERROR)
  ...
  SUBROUTINE POISSON_EQUATION_FINITE_ELEMENT_CALCULATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    REAL(DP) :: CONDUCTIVITY_MATERIAL(3,3),CONDUCTIVITY(3,3),CONDUCTIVITY_TEMP(3,3) !! THE CONDUCTIVITY FOR CONSTANT SOURCE POISSON
...
!conductivity in material coordinates 
            ...
            ENDIF
            IF(ASSOCIATED(FIBRE_FIELD)) THEN !rotate the conductivities from material coordinates (nu) to xi-space to get the effective conductivity
  ...
  SUBROUTINE POISSON_EQUATION_PROBLEM_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_QUASISTATIC_LINEAR_SOURCE_POISSON_SUBTYPE)
        CALL POISSON_EQUATION_PROBLEM_QUASISTATICLINEAR_SOURCE_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*999)
  ...
  SUBROUTINE POISSON_EQUATION_PROBLEM_SUBTYPE_SET(PROBLEM,PROBLEM_SUBTYPE,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      CASE(PROBLEM_QUASISTATIC_LINEAR_SOURCE_POISSON_SUBTYPE)        
        PROBLEM%CLASS=PROBLEM_CLASSICAL_FIELD_CLASS
        PROBLEM%TYPE=PROBLEM_POISSON_EQUATION_TYPE
        PROBLEM%SUBTYPE=PROBLEM_QUASISTATIC_LINEAR_SOURCE_POISSON_SUBTYPE     
      CASE(PROBLEM_NONLINEAR_SOURCE_POISSON_SUBTYPE)        
        PROBLEM%CLASS=PROBLEM_CLASSICAL_FIELD_CLASS
        PROBLEM%TYPE=PROBLEM_POISSON_EQUATION_TYPE
...
999 CALL ERRORS("POISSON_EQUATION_PROBLEM_LINEAR_SOURCE_SETUP",ERR,ERROR)
  ...
  SUBROUTINE POISSON_EQUATION_PROBLEM_QUASISTATICLINEAR_SOURCE_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*)
    ...
    CALL ENTERS("POISSON_EQUATION_PROBLEM_QUASISTATICLINEAR_SOURCE_SETUP",ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      IF(PROBLEM%SUBTYPE==PROBLEM_QUASISTATIC_LINEAR_SOURCE_POISSON_SUBTYPE) THEN
        SELECT CASE(PROBLEM_SETUP%SETUP_TYPE)
        CASE(PROBLEM_SETUP_INITIAL_TYPE)
          SELECT CASE(PROBLEM_SETUP%ACTION_TYPE)
          CASE(PROBLEM_SETUP_START_ACTION)
          ...
          CASE DEFAULT
            LOCAL_ERROR="The action type of "//TRIM(NUMBER_TO_VSTRING(PROBLEM_SETUP%ACTION_TYPE,"*",ERR,ERROR))// &
              & " for a setup type of "//TRIM(NUMBER_TO_VSTRING(PROBLEM_SETUP%SETUP_TYPE,"*",ERR,ERROR))// &
              & " is invalid for a QUASISTATIC_LINEAR source Poisson equation."
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT
        CASE(PROBLEM_SETUP_CONTROL_TYPE)
          SELECT CASE(PROBLEM_SETUP%ACTION_TYPE)
          ...
          CASE DEFAULT
            LOCAL_ERROR="The action type of "//TRIM(NUMBER_TO_VSTRING(PROBLEM_SETUP%ACTION_TYPE,"*",ERR,ERROR))// &
              & " for a setup type of "//TRIM(NUMBER_TO_VSTRING(PROBLEM_SETUP%SETUP_TYPE,"*",ERR,ERROR))// &
              & " is invalid for a QUASISTATIC_LINEAR source equation."
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT
        CASE(PROBLEM_SETUP_SOLVERS_TYPE)
          !Get the control loop
    ...
    CALL EXITS("POISSON_EQUATION_PROBLEM_QUASISTATIC_LINEAR_SOURCE_SETUP")
    RETURN
999 CALL ERRORS("POISSON_EQUATION_PROBLEM_QUASISTATIC_LINEAR_SOURCE_SETUP",ERR,ERROR)
    CALL EXITS("POISSON_EQUATION_PROBLEM_QUASISTATIC_LINEAR_SOURCE_SETUP")
    RETURN 1
  END SUBROUTINE POISSON_EQUATION_PROBLEM_QUASISTATICLINEAR_SOURCE_SETUP
  !================================================================================================================================
  !
...
999 CALL ERRORS("POISSON_PRE_SOLVE_UPDATE_INPUT_DATA",ERR,ERROR)
  ...
  !================================================================================================================================
  !
  !Update the boundary conditions FOR QUASISTATIC_LINEAR SOURCE POISSON
...
!\todo: Reduce number of variable used
    ...
    INTEGER(INTG) :: local_ny,global_ny,NUM_BC_NODES,I,ITERATION
    ...
    IF(ASSOCIATED(CONTROL_LOOP)) THEN
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
          SELECT CASE(CONTROL_LOOP%PROBLEM%SUBTYPE)
          CASE(PROBLEM_EXTRACELLULAR_BIDOMAIN_POISSON_SUBTYPE)
            CALL FLAG_ERROR("Boundary condition is not updated for bidomain sub-type.",ERR,ERROR,*999)
          CASE(PROBLEM_LINEAR_SOURCE_POISSON_SUBTYPE)
            CALL FLAG_ERROR("Boundary condition is not updated for linear sub-type.",ERR,ERROR,*999)
          CASE(PROBLEM_NONLINEAR_SOURCE_POISSON_SUBTYPE)
            CALL FLAG_ERROR("Boundary condition is not updated for non_linear sub-type.",ERR,ERROR,*999)
          CASE (PROBLEM_QUASISTATIC_LINEAR_SOURCE_POISSON_SUBTYPE)
            SOLVER_EQUATIONS=>SOLVER%SOLVER_EQUATIONS
            IF(ASSOCIATED(SOLVER_EQUATIONS)) THEN
              SOLVER_MAPPING=>SOLVER_EQUATIONS%SOLVER_MAPPING
                    ...
                    IF(ASSOCIATED(DEPENDENT_FIELD)) THEN
                      ...
                      IF(ASSOCIATED(FIELD_VARIABLE)) THEN
                        ...
                        IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE)) THEN
                          ...
                          CALL WRITE_STRING(GENERAL_OUTPUT_TYPE,"  reading the boundary values from file... ",ERR,ERROR,*999)
                          DO I=1,NUM_BC_NODES
                ...
                IF(ASSOCIATED(SOLVER_MAPPING)) THEN
                  ...
                  DO equations_set_idx=1,SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS
                    ...
                    IF(TIME_LOOP%TIME_LOOP%CURRENT_TIME<=TIME_LOOP%TIME_LOOP%STOP_TIME) THEN
                      ...
                      FILE_NAME="./output/"//"TIMESTP_"//TRIM(NUMBER_TO_VSTRING(CURRENT_LOOP_ITERATION,"*",ERR,ERROR))
                      METHOD="FORTRAN"
                      EXPORT_FIELD=.TRUE.
...
!          FILE="TRANSIENT_OUTPUT"
            ...
            CASE DEFAULT
              LOCAL_ERROR="Problem subtype "//TRIM(NUMBER_TO_VSTRING(CONTROL_LOOP%PROBLEM%SUBTYPE,"*",ERR,ERROR))// &
                & " is not valid for a Poisson equation fluid type of a classical field problem class."
              CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT
        ELSE
          CALL FLAG_ERROR("Problem is not associated.",ERR,ERROR,*999)
